...
if __name__ == "__main__":
    ...
    mp_mixedflow.od_pts = ['OD_35K_080M_T4_3100', 'OD_35K_080M_T4_3000', 'OD_35K_080M_T4_2900', 'OD_35K_080M_T4_2800', 'OD_35K_080M_T4_2700']
    mp_mixedflow.od_T4s = [3100, 3000, 2900, 2800, 2700]
    mp_mixedflow.od_alts = [35000, 35000, 35000, 35000, 35000]
    mp_mixedflow.od_MNs = [0.8, 0.8, 0.8, 0.8, 0.8]

    # Define off design points
    # mp_mixedflow.od_pts = ['ThreeHundred300']
    # mp_mixedflow.od_T4s = [3000]
    # mp_mixedflow.od_alts = [30000]
    # mp_mixedflow.od_MNs = [0.8]
    ...
    for i, pt in enumerate(mp_mixedflow.od_pts):
        mp_mixedflow.pyc_add_pnt(pt, MixedFlowTurbofan(design=False, thermo_method='CEA'))

        mp_mixedflow.set_input_defaults(f'{pt}.balance.rhs:FAR_core', mp_mixedflow.od_T4s[i], units='degR')
        mp_mixedflow.set_input_defaults(f'{pt}.fc.alt', mp_mixedflow.od_alts[i], units='ft')
        mp_mixedflow.set_input_defaults(f'{pt}.fc.MN', mp_mixedflow.od_MNs[i])

        # Apply shared parameters explicitly to the OD points
        for param, val, units in shared_params:
            if units:
                mp_mixedflow.set_input_defaults(f'{pt}.{param}', val, units=units)
            else:
                mp_mixedflow.set_input_defaults(f'{pt}.{param}', val)
    ...
    csv_filename = "mixedflow_turbofan_30K_080MN_PPL_test.csv"
    df.to_csv(csv_filename)

    csv_reduced_filename = "mixedflow_turbofan_30K_080MN_PPL_reduced_test.csv"
